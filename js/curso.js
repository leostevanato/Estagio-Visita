...
let curso_turma_texto = ((configuracoesJSON.turma.toLowerCase().includes("turma ")) ? "" : "TURMA ") + "<span>" + configuracoesJSON.turma.toUpperCase() + "</span>";

const cursoLogoList = document.querySelectorAll('.item-conteudo > .curso-logo');

for (let i = 0; i < cursoLogoList.length; i++) {
	let curso_turma = document.createElement("span");
	curso_turma.innerHTML = curso_turma_texto;
	curso_turma.classList.add("curso-turma-texto");
	
	cursoLogoList[i].after(curso_turma);
}
...
const carregando = document.querySelector("#curso-container .carregando");

carregando.addEventListener('transitionend', () => {
	carregando.remove();
});

carregando.style.transition = "all 1s";
